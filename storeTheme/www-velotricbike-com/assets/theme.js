...
(function(){
  ...
  theme.pageTransitions = function() {
    if (document.body.dataset.transitions === 'true') {
      ...
      document.querySelectorAll('a[href*="#"], a[class*="jdgm"], a[href^="#"], a[target="_blank"], a[href*="youtube.com/watch"], a[href*="youtu.be/"], a[download]').forEach(el => {
  ...
  theme.AjaxProduct = (function() {
    ...
    function ProductForm(form, submit, args) {
      ...
      if(this.form.querySelectorAll(".shopify-payment-button").length > 0){
        var shopifyPaymentButton = this.form.querySelector(".shopify-payment-button");
        shopifyPaymentButton.addEventListener("DOMSubtreeModified", function() {
          var buyButton = shopifyPaymentButton.querySelector(".shopify-payment-button__button");
          if(buyButton && buyButton.innerHTML == "Buy it now") {
            buyButton.innerHTML = "Shop Now";
          }
        })
      }
    };
  ...
  theme.Slideshow = (function() {
    ...
    var defaults = {
      ...
      autoPlay: false,
      draggable: false, // Disable drag feature by Tan
  ...
  theme.predictiveSearch = (function() {
    ...
    function buildProducts(results) {
      ...
      if (products.length) {
        var markup = theme.buildProductGridItem(products, 'one-whole');
  ...
  theme.SlideshowSection = (function() {
    ...
    SlideshowSection.prototype = Object.assign({}, SlideshowSection.prototype, {
      ...
      onSliderInit: function(){
        var slideshow_wrapper = document.querySelector('.slideshow-wrapper');
        slideshow_wrapper.querySelectorAll('.flickity-button-icon').forEach(function(elem, index){
          elem.setAttribute('viewBox', '0 0 18 34');
        });
      },
  ...
  theme.Product = (function() {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      specHtml : function() { 
        var origin_spec_code =$(".spec-content").html();
        if(origin_spec_code != null){
          var arrs= origin_spec_code.split("<p>&nbsp;</p>");
          var arrs_len = arrs.length;
          var new_arr = [];
          var groupstr="";
          var groupspace=Math.ceil(arrs_len/2);
          for(var i=0;i<arrs_len;i+=groupspace){
            new_arr.push(arrs.slice(i,i+groupspace).join(""));
          }   
          var new_arrs_len = new_arr.length;  
          for(var j=0;j<new_arrs_len;j++){
            groupstr +="<div class='table-item'>" + new_arr[j] +"</div>";
          }
          $(".spec-content").html(groupstr);
        }
      },
      ...
      initVariants: function() {
        ...
        this.container.querySelectorAll(this.selectors.variantColorSwatch).forEach(swatch => {
          swatch.addEventListener('change', function(evt) {
            ...
            el.parentElement.parentElement.querySelectorAll('.variant-input').forEach(function(elem){
      ...
      initProductSlider: function(variant) {
        ...
        var mainSliderArgs = {
          ...
          wrapAround: false,
          prevNextButtons: true,
          arrowShape: 'm0.26697,50.02179l33.93256,-31.06029l-0.1494,28.43233l65.53908,0.2575l0.09957,6.79401l-65.72028,0.4015l0.25082,29.23087l-33.95235,-34.05592z',
          pageDots: true,
          callbacks: {
            onInit: this.onSliderInit.bind(this),
  ...
  DOMready(function(){
    ...
    if(document.querySelectorAll('.testimonial-box-container').length > 0){
      document.querySelectorAll('.professional-testimonial-container').forEach(function(element){ 
        var swiper_elem_1 = element.querySelector('.testimonial-box-container');
        var swiper_elem_2 = element.querySelector('.testimonial-thumbs');
        var pagination_elem = element.querySelector('.swiper-pagination');
        ...
        //   }
        // });
        var swiper = new Swiper(swiper_elem_2, {
          spaceBetween: 0,
          slidesPerView: 4,
          freeMode: true,
          autoHeight: true,
        ...
        var swiper2 = new Swiper(swiper_elem_1, {
          spaceBetween: 0,
          slidesPerView: 1,
          slidesPerGroup: 1,
          autoHeight: true,
    ...
    if(document.querySelectorAll('.features-review-section-wrapper').length > 0){
      document.querySelectorAll('.features-review-section-wrapper').forEach(function(element){ 
        var swiper_elem = element.querySelector('.swiper');
        var per_row = element.getAttribute('data-per-row');
        var pagination_elem = element.querySelector('.swiper-pagination');
        var prev_nav_elem = element.querySelector('.swiper-button-prev');
        var next_nav_elem = element.querySelector('.swiper-button-next');
        new Swiper(swiper_elem, {
          // Optional parameters
          loop: true,
          slidesPerView: 1,
          ...
          navigation: {
            nextEl: next_nav_elem,
            prevEl: prev_nav_elem,
          },
          breakpoints: {
            790: {
              slidesPerView: 2,
...
$('.product-save .save-btn').click(function(){
  $('.popup-container').addClass("active");
  $('.header-wrapper--overlay').addClass('popup-active');
});
$('.popup-container').click(function(evt){
  if($(evt.target).parents(".popup-inner").length==0) {
    $(this).removeClass("active");
    $('.header-wrapper--overlay').removeClass('popup-active');
  }
});
$('.popup-inner .inner-top .popup-close-btn').click(function(){
  $('.popup-container').removeClass("active");
  $('.header-wrapper--overlay').removeClass('popup-active');
});

$(".service-btn").click(function(){
  if($(".service-popup").is(":hidden")){
    $(".service-popup").fadeIn();
  }else{
    $(".service-popup").fadeOut();
  }     
});
$("body").bind("click",function(evt){       
       if($(evt.target).parents(".service-window").length==0 && $(evt.target).closest(".service-popup").length==0 ) 
       { 
         $(".service-popup").fadeOut();          
       } 
});
...
// 
// [Start] Custom Pre Order on Featured Product Section added by Alfie from InsCoder (6.24.2022)
// 

// const featuredProductSectionAtcButtonText = $('.featured-product-section [data-add-to-cart-text]')
// const featuredProductSectionVariantInput = $('.featured-product-section [data-variant-input]')
// const featuredProductSectionVariantInputSelected = $('.featured-product-section [data-variant-input]:checked')
// const featuredProductSectionMessage = $('.featured-product-section--pre-order-message')
// const featuredProductSectionVariantInputSelectedAvailability = featuredProductSectionVariantInputSelected.data('variant-availability')
// const featuredProductSectionVariantInputSelectedValue = featuredProductSectionVariantInputSelected.val()
// const featuredProductSectionAtcButtonDefaultText = featuredProductSectionAtcButtonText.data('default-text')
// const featuredProductSectionAtcButtonPreOrderText = featuredProductSectionAtcButtonText.data('pre-order-text')
// let jsonFeaturedProductSection = JSON.parse(document.getElementById('preorder-message').textContent)

// function isNotAvailable(num) {
//   if (num <= 0) return true; 
// }

// function getPreOrderMessage(variant) {
//   const message = jsonFeaturedProductSection[variant].message != '' ? jsonFeaturedProductSection[variant].message : ''
//   return message
// }

// function toggleAtcButtonText(inventory, variant) {

//   const variantValue = variant.replace(/ /g,'')
//   console.log('variantValue', variantValue)

//   if(isNotAvailable(inventory)) {
//     featuredProductSectionAtcButtonText.text(featuredProductSectionAtcButtonPreOrderText)
//     featuredProductSectionMessage.removeClass('hide')
//     featuredProductSectionMessage.text(getPreOrderMessage(variant))
//   } else {
//     featuredProductSectionAtcButtonText.text(featuredProductSectionAtcButtonDefaultText)
//     featuredProductSectionMessage.addClass('hide')
//   }
// }

// if(featuredProductSectionAtcButtonText.length) {
//   toggleAtcButtonText(featuredProductSectionVariantInputSelectedAvailability, featuredProductSectionVariantInputSelectedValue)

//   featuredProductSectionVariantInput.on('change', function(){
//     toggleAtcButtonText($(this).data('variant-availability'), $(this).val())
//   })
// }

// [End] Custom Pre Order on Featured Product Section