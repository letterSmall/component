...
(function(){
    ...
    if (document.body.dataset.transitions === 'true') {
      ...
      document.querySelectorAll('a:not(.js-no-transition)').forEach(el => {
        el.addEventListener('click', function(evt) {
          if(window.location.href.indexOf('help-center')>-1){if(evt.target.closest('.s4com-helpcenter')){return;}}
  ...
  DOMready(function(){
    ...
    if (document.querySelector('.featured-posts.medium-down--show') && matchMedia('screen and (max-width: 769px)').matches) {
      new theme.Slideshow(document.querySelector('.featured-posts.medium-down--show'), {
        prevNextButtons: false,
        pageDots: true,
        fade: false,
        autoPlay: false
...
// product introduction topbar js
$(document).ready(function(){
  $(window).scroll(function(){
    if($(window).scrollTop() > $("#shopify-section-header").offset().top + 50){
      $("#StickyHeaderWrap").css("opacity","0");
      $("#pi_topbar").css("top","0");
    }else{
      $("#StickyHeaderWrap").css("opacity","1");
      $("#pi_topbar").css("top","-130px");
    }
  });
});