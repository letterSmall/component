  ...
  theme.Drawers = (function() {
    ...
    Drawer.prototype = $.extend({}, Drawer.prototype, {
      ...
      open: function(evt, returnFocusEl) {
        ...
        let name= $(evt.currentTarget).attr('aria-controls');
        if(name=='CartDrawer'){
          $('.site-nav-box').addClass('other');
        }
        if (evt) {
          evt.preventDefault();
        }
        ...
        let header_height=0;
           if(!headerIsLight){
           header_height=$('#site-header').outerHeight(true)+$('.announcement').outerHeight(true);
          }
        else{
          header_height=$('#site-header').outerHeight(true);
        }
        
         
        let header_status=$('.header-wrapper').hasClass('is-light');
        if($(window).width()<=589){
          if(header_status){
           $('.header-wrapper').removeClass('is-light');
          }
       
          $('.drawer').css({'top':`${header_height}px`,'height':`calc(100% - ${header_height}px`});
        $('#site-header').addClass('site-header--bg-white');
        }
        ...
        this.isOpen = true;
//         $(".bg_navss").addClass("active");
//         $("#nav-mmenus").addClass("current");
//          $(".drawer-menu-bottom-container").css("display","block");
//          $(".nav-mmenus-menu").css("display","block");
        theme.a11y.trapFocus({
      ...
      close: function() {
        ...
        $('.site-nav-box').removeClass('other');
           let header_status=$('.header-wrapper').hasClass('is-light');
      
         if(header_status){
           $('.header-wrapper').addClass('is-light');
          }
        
        $('#site-header').removeClass('site-header--bg-white');
        if (!this.isOpen) {
          return;
        }
  ...
  theme.CartDrawer = (function() {
    ...
    CartDrawer.prototype = $.extend({}, CartDrawer.prototype, {
      ...
      updateCartNotification: function(evt, cart) {
        if (cart.item_count > 0) {
          $(selectors.cartBubble).text(cart.item_count > 99 ? '99+' : cart.item_count)
          $(selectors.cartBubble).addClass('cart-link__bubble--visible');
        } else {
          $(selectors.cartBubble).text('')
  ...
  theme.headerNav = (function() {
    ...
    function init() {
      ...
      if (theme.settings.overlayHeader && Shopify.designMode) {
...
//       $parent.addClass('js-drawer-open js-drawer-open--search');
    ...
    function accessibleDropdowns() {
      ...
      function openFirstLevelDropdown($el) {
        var $parentItem = $el.parent();
        let name=$parentItem.data('name');
        if ($parentItem.hasClass(classes.hasDropdownClass)) {
          $parentItem.addClass(classes.dropdownActive);
     
          if(name="shop"){
//             let nav_width=$('#site-navigation').outerWidth(true);
//           $('.site-nav__expanded-item-shop .snd-inner').css('width',`${nav_width-40}px`);
          }
    ...
    function stickyHeaderScroll() {
      ...
      if (scroll > threshold) {
        ...
        if($(window).width()>589){
          $('.pc-product-cart-container').addClass(config.stickyClass);
             setTimeout(function() {
          $siteHeader.addClass(config.openTransitionClass);
              
            $('.pc-product-cart-container').addClass(config.openTransitionClass);
                   $('.pc-product-cart-container').css('transform',` translateY(${header_height}px)`)
        }, 100);
        ...
        config.stickyActive = false;
         headerIsLight=false;
    $('.pc-product-cart-container').css('transform',` translateY(0px)`)
      
 
         
        $('.pc-product-cart-container').removeClass(config.openTransitionClass).removeClass(config.stickyClass);
      
        $siteHeader.removeClass(config.openTransitionClass).removeClass(config.stickyClass);
        if (config.wrapperOverlayed&&!headerIsLight) {
  ...
  theme.announcementBar = (function() {
    ...
    function init() {
      ...
      if (theme.config.hasSessionStorage && sessionStorage[announcementBarText()] !== 'hidden') {
        window.setTimeout(function() {
          announcementBarShow();
               let header_height=$('#site-header').outerHeight(true)+$('.announcement').outerHeight(true);
          $('.drawer').css({'top':`${header_height}px`,'height':`calc(100% - ${header_height}px`});   
        }, 2000);
      }
  ...
  theme.Product = (function() {
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      createImageCarousels: function() {
        ...
        console.log(thumbnailsVertical);
      ...
      mainSlideInit: function(event, slick) {
        ...
        if ($video.length) {
          this.initVideo($video);
               ...
               var slide_fliter=1;
$('.pifb-inner .item').click(function(){
  $(this).addClass('active').siblings().removeClass('active');
  let type=$(this).data('type');
  console.log(type);
  
//    $('.product-image-container').slick('unslick');
//    $('.product__main-photos .slick-slide').hide();
//   $(`.product__main-photos .slick-slide.slide-${type}`).show();
//   $('.product-image-container').slick();
  
    $slider.slick('slickUnfilter');
 
               $slider.slick('slickFilter',".slide-" + type);
  $slider.slick('slickGoTo', 0);
    var $currentSlide = $slider.find(selectors.currentSlide);
        var $video = $currentSlide.find(selectors.productVideo);
  if ($video.length) {
          that.initVideo($video);
        }
      ...
      thumbSlideArrowVisibility: function(event, slick) {
        ...
        if (this.settings.thumbVertical) {
          ...
          console.log($slider.find('.slick-track').width());
      ...
      init: function() {
        ...
        $placeholder.load(url + this.selectors.sectionClass, function(data) {
          ...
          if ($(data).find(this.selectors.sectionClass).find(this.selectors.productResults).length === 0) {
           ...
           var recommendations_Swiper = new Swiper('.recommendations-swiper-container', {
observer: true,
observeParents: true,
freeMode: true,
 navigation: {
      ...
      drawerMenuButtons: function() {
        $(selectors.drawer).find('.js-drawer-close').on('click', function(evt){
          ...
          theme.NavDrawer.close();
           $("#nav-mmenus").removeClass("current");
        });
   $(".js-search-header-close").click(function(){
 theme.NavDrawer.close(); $("#nav-mmenus").removeClass("current");
    });
    $(".bg_navss").click(function(){
      theme.NavDrawer.close();
      $(".bg_navss").removeClass("active");
       $("#nav-mmenus").removeClass("current");
    });
  ...
  theme.pageTransitions = function() {
    if ($('body').data('transitions') == true) {
      ...
      $('a:not(.js-no-transition, .jdgm-paginate__page, .jdgm-star, .jdgm-rev__body-read-more)').bind('click', function(evt) {
...
$(document).ready(function(){$('.Footer-newsletter-mb').click();$('.grid__item--logo_social').before('<div class="grid grid__item custom_grid_item_footer"></div>');$('.custom_grid_item_footer').append($('.grid__item--logo_social'));$('.custom_grid_item_footer').append($('.grid__item--newsletter'))});
$(document).ready(function(){if($('#psc-tabbar').offset()){let site_header_el=$('#psc-tabbar')||"";let site_header_position=site_header_el.data('position');let announcement_bar=site_header_el.offset().top-$('.site-header').outerHeight(true);let scroll_num=1;$(document).scroll(function(){t=$(window).scrollTop();if(scroll_num==1){announcement_bar=site_header_el.offset().top-$('.site-header').outerHeight(true)}scroll_num++;if(t>announcement_bar){site_header_el.addClass('sticky');site_header_el.css('top',`${$('.site-header').outerHeight(true)}px`)}else{site_header_el.removeClass('sticky');site_header_el.css('top',`0px`)}})}});
$('.warranty-content .option-item').each(function(){let title=$(this).find('.title').text();$(this).find('.title').data('status',false);let title_html=`<span>${title}</span><span class="icon-box"><svg t="1625025564018"class="icon"viewBox="0 0 1024 1024"version="1.1"xmlns="http://www.w3.org/2000/svg"p-id="2646"width="200"height="200"><path d="M959.429379 343.214852 890.590548 274.378068 511.268336 653.699256 131.944078 274.378068 63.105247 343.214852 501.1857 781.294282 521.348925 781.294282Z"p-id="2647"></path></svg></span>`;$(this).find('.title').html(title_html)});
$('.warranty-content .content span').each(function(){let content=$(this).text();let first="",last="";let content_html="";if(content.indexOf(":")!=-1){first=content.split(':')[0];last=content.split(':')[1];content_html=`<div class="cell"><span>${first}</span>:<span>${last}</span></div>`;$(this).html(content_html)}if(content.indexOf("：")!=-1){first=content.split('：')[0];last=content.split('：')[1];content_html=`<div class="cell"><span>${first}</span>:<span>${last}</span></div>`;$(this).html(content_html)}});
$('.warranty-content .title').click(function(){$(this).next('.content').slideToggle();let status=$(this).data('status');if(!status){$(this).find('.icon-box').addClass('rotate');$(this).data('status',true);return;}else{$(this).find('.icon-box').removeClass('rotate');$(this).data('status',false);return;}});
$('.mb-psc-tabbar-item').click(function(){let name=$(this).data('name');$(`.psc-inner-content-${name}`).slideToggle();let status=$(this).data('status');if(!status){$(this).find('.icon-box').addClass('rotate');$(this).data('status',true);return;}else{$(this).find('.icon-box').removeClass('rotate');$(this).data('status',false);return;}});
// $('.psc-tabbar-item ').click(function(){let name=$(this).data('name');$(this).addClass('active').siblings().removeClass('active');if(name=="review"){$("html, body").animate({scrollTop:$(`.product-desc-contianer#judgeme_product_reviews`).offset().top-$('.site-header').outerHeight(true)-$('.psc-tabbar').outerHeight(true)},500)}else{$("html, body").animate({scrollTop:$(`.psc-content-item-${name}`).offset().top-$('.site-header').outerHeight(true)-$('.psc-tabbar').outerHeight(true)},500)}return false})
function initdescSwiper(){$('.pscs3-content ').wrap('<div class="swiper-container pscs3-swiper-container">');$('.pscs3-content ').addClass('swiper-wrapper');$('.pscs3-content .item').addClass('swiper-slide');$('.pscs3-swiper-container').append(`<div class="swiper-button-prev"><svg aria-hidden="true"focusable="false"role="presentation"class="icon icon-chevron-left"viewBox="0 0 284.49 498.98"><path d="M249.49 0a35 35 0 0 1 24.75 59.75L84.49 249.49l189.75 189.74a35.002 35.002 0 1 1-49.5 49.5L10.25 274.24a35 35 0 0 1 0-49.5L224.74 10.25A34.89 34.89 0 0 1 249.49 0z"></path></svg></div><div class="swiper-button-next"><svg aria-hidden="true"focusable="false"role="presentation"class="icon icon-chevron-right"viewBox="0 0 284.49 498.98"><path d="M35 498.98a35 35 0 0 1-24.75-59.75l189.74-189.74L10.25 59.75a35.002 35.002 0 0 1 49.5-49.5l214.49 214.49a35 35 0 0 1 0 49.5L59.75 488.73A34.89 34.89 0 0 1 35 498.98z"></path></svg></div>`);let pscs3Swiper="";pscs3Swiper=new Swiper('.pscs3-swiper-container',{observer:true,observeParents:true,slidesPerView:2,centeredSlides:false,allowTouchMove:false,breakpoints:{590:{slidesPerView:1,allowTouchMove:true,navigation:{nextEl:'.swiper-button-next',prevEl:'.swiper-button-prev',},}}})}
initdescSwiper();
$(document).ready(function(){let href=window.location.href;if(href.indexOf('challenge#newsletter-footer')!=-1){$("html, body").animate({scrollTop:0},000)}if(href.indexOf('customer_posted')!=-1||href.indexOf('form_type=customer')!=-1){if($("#newsletter-footer .errors").length<2){$('.subscribed-tips').removeClass('hide');setTimeout(function(){$('.subscribed-tips').addClass('hide')},6000)}$("html, body").animate({scrollTop:$(`.footer__newsletter`).offset().top},000)}});
$(document).ready(function(){if($('.specs-content img').length>=1){$('.specs-content').addClass('img');}});
new theme.AjaxProduct($('.pccc-product-single__form'));
$(document).ready(function(){let zheng_height= $('.pc-product-cart-container').outerHeight(true);$('.pc-product-cart-container-shadow').css('height',`${zheng_height}px`);});
$(document).ready(function(){let mpspb_nunm=$('.mpspb-swiper-container').data('num'),allowTouchMovePc=false,allowTouchMoveMb=false;if(mpspb_nunm>4){allowTouchMovePc=true}if(mpspb_nunm>2){allowTouchMoveMb=true}var psSwiper=new Swiper('.mpspb-swiper-container',{slidesPerView:4,spaceBetween:30,allowTouchMove:allowTouchMovePc,navigation:{nextEl:'.swiper-button-next',prevEl:'.swiper-button-prev',},breakpoints:{768:{slidesPerView:2,allowTouchMove:allowTouchMoveMb,spaceBetween:22,}}})});
$('.sndi-left li').hover(function(){if($(window).width()<=768)return; let index=$(this).data('index');$(`.has_sub_dropdown-box`).hide();$(`.has_sub_dropdown-box-${index}`).show();$(this).find("a").css("color","#1b1b1b");$(this).siblings().find("a").css("color","#999999")},function(){if($(window).width()<=768)return; let index=$(this).data('index');$(this).siblings().find("a").css("color","#1b1b1b")});
$('.site-footer__linklist li').hover(function(){if($(window).width()<=768)return; let index=$(this).data('index')});
$('.site-nav__deep-dropdown li').hover(function(){if($(window).width()<=768)return; let index=$(this).data('index');let parindex=$(this).data('parindex');if($(`.snddib-li-${parindex}-${index}`).length>0){$(`.snddib-li-${parindex}-${index}`).stop(false,true).fadeIn().siblings().stop(false,true).fadeOut()}else{$(`.has_sub_dropdown-box-${parindex}.snddib-li`).stop(false,true).fadeOut()}$(this).find("a").css("color","#1b1b1b");$(this).siblings().find("a").css("color","#999999");$(`.sndi-left>.sndt-${parindex}>a`).addClass("activeho");$(`.sndi-left>.sndt-${parindex}`).siblings().find("a").css("color","#999999")},function(){if($(window).width()<=768)return; let index=$(this).data('index');let parindex=$(this).data('parindex');if($(`.snddib-li-${parindex}-${index}`).length>0){$(`.snddib-li-${parindex}-${index}`).stop(false,true).fadeIn().siblings().stop(false,true).fadeOut()}else{$(`.has_sub_dropdown-box-${parindex}.snddib-li`).stop(false,true).fadeOut()}$(`.sndi-left>.sndt-${parindex}>a`).removeClass("activeho");$(this).siblings().find("a").css("color","#1b1b1b");$(`.sndi-left>.sndt-${parindex}`).siblings().find("a").css("color","#1b1b1b")});
let arrowSetStatus=true;
$(document).ready(function(){let status=$('#hero-slideshow-wrapper').data('status')||"";if(status){var arrowSetInterval=setInterval("downArrow()",500)}})
function downArrow(){if(arrowSetStatus){$('.i-go-arrow-1').addClass('active');$('.i-go-arrow-2').removeClass('active');arrowSetStatus=false;return;}else{$('.i-go-arrow-2').addClass('active');$('.i-go-arrow-1').removeClass('active');arrowSetStatus=true;return;}}
$('.go-down-container').click(function(){let el_height=$('#hero-slideshow-wrapper').outerHeight(true)+$('#hero-slideshow-wrapper').offset().top;$("html, body").animate({scrollTop:el_height},400);return false});
$('.Sublinklist-collapsible-trigger').click(function(){setTimeout(()=>{let status=$(this).attr('aria-expanded');if(status=="true"){$(this).parent().parent('.mobile-nav__item').addClass('is-active')}else{$(this).parent().parent('.mobile-nav__item').removeClass('is-active')}},10)});
$('.has_sub_dropdown-box').hover(function(){let name=$(this).data('parent')},function(){let name=$(this).data('parent')});
$(".js-drawer-open-nav").click(function(){$(".bg_navss").addClass("active");$("#nav-mmenus").addClass("current")});
$(document).ready(function(){if($(window).width()>589){$('.product-photos-grid__item').append($(".product-photos-outer"))}else{$('.last-second-item').append($('.product__buy-box .js-qty__wrapper'));$('.product-single-top').after($(".product-photos-outer"))}});
// account
$('.account-item-content .input-box .input-full').focus(function(){if($(this).val()==""){$(this).parent().addClass('active')}});
$('.account-item-content .input-box .input-full').blur(function(){if($(this).val()==""){$(this).parent().removeClass('active')}});
$(document).ready(function(){$('.account-item-content .input-box .input-full').each(function(){if($(this).val()!=""){$(this).parent().addClass('active')}else{$(this).parent().removeClass('active')}})});

// Custom Fullscreen Video Autoplay
$(function(){
  if($(`.home-fullscreen-video.autoplay`).length > 0){
    var mpfblocksLen = $(`.home-fullscreen-video.autoplay .mpf-media-item`).length;
    var youtubeblocksLen = $(`.home-fullscreen-video.autoplay .youtube-media-item`).length;
    var mpf_playArr = [];
    if(mpfblocksLen){
      for(let i =0; i<mpfblocksLen; i++) {
        var tempSectionId = $(`.home-fullscreen-video.autoplay .mpf-media-item`).eq(i).attr("data-section-id");
        var el = document.getElementById(`fullscreen-myMPFour-${tempSectionId}`);
        mpf_playArr[i] = el;
      }
    }
    $(window).scroll(function(){
      $(".home-fullscreen-video.autoplay").each(function(){
        if($(window).scrollTop() + ($(window).height())*5/6 > $(this).offset().top && $(window).scrollTop() + ($(window).height())*1/3 < ($(this).offset().top + $(this).height())){
          $(this).find(`.video-overlap-image`).click();
        } else {
          $(this).find(`.video-overlap-image`).removeClass("active");
          if(mpfblocksLen) {
            for(let j=0; j<mpfblocksLen; j++) {
              mpf_playArr[j].pause();
              mpf_playArr[j].currentTime = 0;
            }
            $(this).find(`.video-overlap-image img`).css("opacity","1");
            $(this).find(`.video-overlap-image img`).css("display","block");
            $(this).find(`.video-overlap-image .button-content-container`).css("display","block");
            $(this).find(`.media-box`).css("display","none");
          }
          if(youtubeblocksLen){
            for(var x = 0; x < youtubeblocksLen; x++) {
              var myPlayer = play_fullscreenVideoArr[x];
              if(myPlayer) {
                myPlayer.pauseVideo();
                myPlayer.seekTo(0);
              }
...
$('.psc-tabbar-item ').click(function() {
  ...
  $(this).addClass('active').siblings().removeClass('active');
  if (name == "review" && $(`.product-desc-contianer#judgeme_product_reviews`).height() > 0) {
    $("html, body, document").animate({
      scrollTop: $(`.product-desc-contianer#judgeme_product_reviews`).offset().top - $('.site-header').outerHeight(true) - $('.psc-tabbar').outerHeight(true)
    },500)
    
  } else {
    if($(`.psc-content-item-${name}`).height() > 0) {
      $("html, body, document").animate({
        scrollTop: $(`.psc-content-item-${name}`).offset().top - $('.site-header').outerHeight(true) - $('.psc-tabbar').outerHeight(true)
      },
                              500)
    }else {
      $("html, body, document").animate({
        scrollTop: $(`.${name}`).eq(0).offset().top - $('.site-header').outerHeight(true) - $('.psc-tabbar').outerHeight(true)
      },
                              500)
    }
  }
